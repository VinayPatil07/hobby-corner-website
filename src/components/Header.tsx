          ...
          <nav className="flex flex-col items-center justify-center h-full space-y-8 pb-20">
            {navItems.map((item) => (
              <Link
                key={item.name}
                to={item.href}
                className="text-2xl font-black uppercase tracking-tighter text-white hover:text-tangerine-accent transition-colors"
              >
                {item.name}
              </Link>
            ))}
            <div className="pt-8 flex gap-8 text-tangerine-accent">
              <Youtube size={24} />
              <Instagram size={24} />
              <Facebook size={24} />
            </div>
